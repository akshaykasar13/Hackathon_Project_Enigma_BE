...
Generate a helpful response for the user.""")
        ...
        print(f"[ResponseAgent] LLM generation failed: {e}, using fallback")
        return None
...
def respond(state):
    ...
    tool_call = observability.log_tool_call("response_generation", "ResponseAgent", {
        ...
        "method": "LLM" if response and not config.TEST_MODE else "fallback"
    })
    observability.log_tool_result(tool_call, {"response_length": len(response), "method": "LLM" if response and not config.TEST_MODE else "fallback"})
    observability.log_agent_execution("ResponseAgent", state, "response_generated")
    
    print(f"[ResponseAgent] Generated response ({'LLM' if response and not config.TEST_MODE else 'fallback'})")
    return state
