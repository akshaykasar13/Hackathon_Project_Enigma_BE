...
def retrieve(state):
    ...
    tool_call = observability.log_tool_call("document_retrieval", "RetrievalAgent", {"query": query[:100]}, state.get("task_id"))
    ...
    try:
        if not query:
            ...
            observability.log_agent_execution("RetrievalAgent", state, "retrieved_0_docs", reasoning="No query provided, skipping retrieval")
            return state
        ...
        reasoning = f"Retrieved {len(docs)} relevant documents from knowledge base" if docs else "No matching documents found in knowledge base"
        observability.log_agent_execution("RetrievalAgent", state, f"retrieved_{len(docs)}_docs", reasoning=reasoning)
        ...
        state["retrieved_docs"] = []
        state["context"] = state.get("context", [])
        observability.log_agent_execution("RetrievalAgent", state, "retrieved_0_docs", reasoning=f"Retrieval failed: {e}")
    return state
