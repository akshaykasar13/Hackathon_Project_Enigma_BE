...
def load_memory(state):
    ...
    # Log tool call
    tool_call = observability.log_tool_call("memory_load", "MemoryAgent", {"task_id": state.get("task_id")}, state.get("task_id"))
    ...
    try:
        ...
        reasoning = f"Loaded {len(working)} working, {len(episodic)} episodic, {len(state.get('semantic_memory', []))} semantic memories"
        observability.log_agent_execution("MemoryAgent", state, "memory_loaded", reasoning=reasoning)
...
def save_memory(state):
    ...
    tool_call = observability.log_tool_call("memory_save", "MemoryAgent", {"ticket": ticket[:100]}, state.get("task_id"))
    ...
    if ticket and response:
        ...
        observability.log_agent_execution("MemoryAgent", state, "memory_saved", reasoning="Saved incident and outcome to episodic memory for future reference")
